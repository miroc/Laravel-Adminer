...
adm_cookie($D,$Y,$ne=2592000){global$ba;return
...
adm_redirect($B,$Ae=null){if($Ae!==null){restart_session();$_SESSION["messages"][preg_replace('~^[^?]*~','',($B!==null?$B:$_SERVER["REQUEST_URI"]))][]=$Ae;}if($B!==null){if($B=="")$B=".";header("Location: $B");exit;}}function
query_adm_redirect($H,$B,$Ae,$pg=true,$Ac=true,$Kc=false,$Oh=""){global$g,$n,$b;if($Ac){$oh=microtime(true);$Kc=!$g->query($H);$Oh=format_time($oh);}$lh="";if($H)$lh=$b->messageQuery($H,$Oh);if($Kc){$n=error().$lh.script("messagesPrint();");return
false;}if($pg)adm_redirect($B,$Ae.$lh);return
true;}function
...
false;}return
true;}function
queries_adm_redirect($B,$Ae,$pg){list($ig,$Oh)=queries(null);return
query_adm_redirect($ig,$B,$Ae,$pg,false,!$pg,$Oh);}function
...
adm_view($D){$L=get_rows('SELECT text "select" FROM user_views WHERE view_name = '.q($D));return
reset($L);}function
collations(){return
array();}function
information_schema($m){return
...
false;}foreach($Ki
as$R){$D=($Hh==DB?table("copy_$R"):idf_escape($Hh).".".table($R));$Ji=adm_view($R);if(!queries("DROP VIEW IF EXISTS $D")||!queries("CREATE VIEW $D AS $Ji[select]"))return
false;}return
true;}function
trigger($D){if($D=="")return
...
selectLinks($zh,$O=""){echo'<p class="links">';$oe=array("select"=>'Select data');if(support("table")||support("indexes"))$oe["table"]='Show structure';if(support("table")){if(is_adm_view($zh))$oe["view"]='Alter view';else$oe["create"]='Alter table';}if($O!==null)$oe["edit"]='New item';foreach($oe
...
drop_create($dc,$i,$ec,$Lh,$gc,$B,$De,$Be,$Ce,$Ze,$Oe){if($_POST["drop"])query_adm_redirect($dc,$B,$De);elseif($Ze=="")query_adm_redirect($i,$B,$Ce);elseif($Ze!=$Oe){$Eb=queries($i);queries_adm_redirect($B,$Be,$Eb&&queries($dc));if($Eb)queries($ec);}else
queries_adm_redirect($B,$Be,queries($Lh)&&queries($gc)&&queries($dc)&&queries($i));}function
...
';}elseif(isset($_GET["dump"])){$a=$_GET["dump"];if($_POST&&!$n){$Bb="";foreach(array("output","format","db_style","routines","events","table_style","auto_increment","triggers","data_style")as$z)$Bb.="&$z=".urlencode($_POST[$z]);adm_cookie("adminer_export",substr($Bb,1));$T=array_flip((array)$_POST["tables"])+array_flip((array)$_POST["data"]);$Gc=dump_headers((count($T)==1?key($T):DB),(DB==""||count($T)>1));$Qd=preg_match('~sql~',$_POST["format"]);if($Qd){echo"-- Adminer $ia ".$cc[DRIVER]." dump\n\n";if($y=="sql"){echo"SET NAMES utf8;
SET time_zone = '+00:00';
...
TmpFile;ob_start(array($Vh,'write'),1e5);}$b->dumpTable($D,($R?$_POST["table_style"]:""),(is_adm_view($S)?2:0));if(is_adm_view($S))$Ki[]=$D;elseif($Jb){$p=fields($D);$b->dumpData($D,$_POST["data_style"],"SELECT *".convert_fields($p,$p)." FROM ".table($D));}if($Qd&&$_POST["triggers"]&&$R&&($ki=trigger_sql($D)))echo"\nDELIMITER ;;\n$ki\nDELIMITER ;\n";if($Gc=="tar"){ob_end_flush();tar_file((DB!=""?"":"$m/")."$D.csv",$Vh);}elseif($Qd)echo"\n";}}foreach($Ki
...
as$D=>$o){if(!isset($o["privileges"][$vi?"update":"insert"])||$b->fieldName($o)=="")unset($p[$D]);}if($_POST&&!$n&&!isset($_GET["select"])){$B=$_POST["referer"];if($_POST["insert"])$B=($vi?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$B))$B=ME."select=".urlencode($a);$x=indexes($a);$qi=unique_array($_GET["where"],$x);$kg="\nWHERE $Z";if(isset($_POST["delete"]))queries_adm_redirect($B,'Item has been deleted.',$bc->delete($a,$kg,!$qi));else{$O=array();foreach($p
as$D=>$o){$X=process_input($o);if($X!==false&&$X!==null)$O[idf_escape($D)]=$X;}if($vi){if(!$O)adm_redirect($B);queries_adm_redirect($B,'Item has been updated.',$bc->update($a,$O,$kg,!$qi));if(is_ajax()){page_headers();page_messages($n);exit;}}else{$I=$bc->insert($a,$O);$ge=($I?last_id():0);queries_adm_redirect($B,sprintf('Item%s has been inserted.',($ge?" $ge":"")),$I);}}}$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$M=array();foreach($p
...
';}elseif(isset($_GET["database"])){$K=$_POST;if($_POST&&!$n&&!isset($_POST["add_x"])){$D=trim($K["name"]);if($_POST["drop"]){$_GET["db"]="";queries_adm_redirect(remove_from_uri("db|database"),'Database has been dropped.',drop_databases(array(DB)));}elseif(DB!==$D){if(DB!=""){$_GET["db"]=$D;queries_adm_redirect(preg_replace('~\bdb=[^&]*&~','',ME)."db=".urlencode($D),'Database has been renamed.',rename_database($D,$K["collation"]));}else{$l=explode("\n",str_replace("\r","",$D));$uh=true;$fe="";foreach($l
as$m){if(count($l)==1||$m!=""){if(!create_database($m,$K["collation"]))$uh=false;$fe=$m;}}restart_session();set_session("dbs",null);queries_adm_redirect(ME."db=".urlencode($fe),'Database has been created.',$uh);}}else{if(!$K["collation"])adm_redirect(substr(ME,0,-1));query_adm_redirect("ALTER DATABASE ".idf_escape($D).(preg_match('~^[a-z0-9_]+$~i',$K["collation"])?" COLLATE $K[collation]":""),substr(ME,0,-1),'Database has been altered.');}}page_header(DB!=""?'Alter database':'Create database',$n,array(),h(DB));$ob=collations();$D=DB;if($_POST)$D=$K["name"];elseif(DB!="")$K["collation"]=db_collation(DB,$ob);elseif($y=="sql"){foreach(get_vals("SHOW GRANTS")as$hd){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$hd,$C)&&$C[1]){$D=stripcslashes(idf_unescape("`$C[2]`"));break;}}}echo'
<form action="" method="post">
<p>
...
';}elseif(isset($_GET["scheme"])){$K=$_POST;if($_POST&&!$n){$A=preg_replace('~ns=[^&]*&~','',ME)."ns=";if($_POST["drop"])query_adm_redirect("DROP SCHEMA ".idf_escape($_GET["ns"]),$A,'Schema has been dropped.');else{$D=trim($K["name"]);$A.=urlencode($D);if($_GET["ns"]=="")query_adm_redirect("CREATE SCHEMA ".idf_escape($D),$A,'Schema has been created.');elseif($_GET["ns"]!=$D)query_adm_redirect("ALTER SCHEMA ".idf_escape($_GET["ns"])." RENAME TO ".idf_escape($D),$A,'Schema has been altered.');else
adm_redirect($A);}}page_header($_GET["ns"]!=""?'Alter schema':'Create schema',$n);if(!$K)$K["name"]=$_GET["ns"];echo'
<form action="" method="post">
...
';}elseif(isset($_GET["foreign"])){$a=$_GET["foreign"];$D=$_GET["name"];$K=$_POST;if($_POST&&!$n&&!$_POST["add"]&&!$_POST["change"]&&!$_POST["change-js"]){$Ae=($_POST["drop"]?'Foreign key has been dropped.':($D!=""?'Foreign key has been altered.':'Foreign key has been created.'));$B=ME."table=".urlencode($a);if(!$_POST["drop"]){$K["source"]=array_filter($K["source"],'strlen');ksort($K["source"]);$Hh=array();foreach($K["source"]as$z=>$X)$Hh[$z]=$K["target"][$z];$K["target"]=$Hh;}if($y=="sqlite")queries_adm_redirect($B,$Ae,recreate_table($a,$a,array(),array(),array(" $D"=>($_POST["drop"]?"":" ".format_foreign_key($K)))));else{$c="ALTER TABLE ".table($a);$dc="\nDROP ".($y=="sql"?"FOREIGN KEY ":"CONSTRAINT ").idf_escape($D);if($_POST["drop"])query_adm_redirect($c.$dc,$B,$Ae);else{query_adm_redirect($c.($D!=""?"$dc,":"")."\nADD".format_foreign_key($K),$B,$Ae);$n='Source and target columns must have the same data type, there must be an index on the target columns and referenced data must exist.'."<br>$n";}}}page_header('Foreign key',$n,array("table"=>$a),h($a));if($_POST){ksort($K["source"]);if($_POST["add"])$K["source"][]="";elseif($_POST["change"]||$_POST["change-js"])$K["target"]=array();}elseif($D!=""){$ad=foreign_keys($a);$K=$ad[$D];$K["source"][]="";}else{$K["table"]=$a;$K["source"]=array("");}$hh=array_keys(fields($a));$Hh=($a===$K["table"]?$hh:array_keys(fields($K["table"])));$rg=array_keys(array_filter(table_status('',true),'fk_support'));echo'
<form action="" method="post">
<p>
...
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$K=$_POST;$uf="VIEW";if($y=="pgsql"&&$a!=""){$P=table_status($a);$uf=strtoupper($P["Engine"]);}if($_POST&&!$n){$D=trim($K["name"]);$Ga=" AS\n$K[select]";$B=ME."table=".urlencode($D);$Ae='View has been altered.';$U=($_POST["materialized"]?"MATERIALIZED VIEW":"VIEW");if(!$_POST["drop"]&&$a==$D&&$y!="sqlite"&&$U=="VIEW"&&$uf=="VIEW")query_adm_redirect(($y=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($D).$Ga,$B,$Ae);else{$Jh=$D."_adminer_".uniqid();drop_create("DROP $uf ".table($a),"CREATE $U ".table($D).$Ga,"DROP $U ".table($D),"CREATE $U ".table($Jh).$Ga,"DROP $U ".table($Jh),($_POST["drop"]?substr(ME,0,-1):$B),'View has been dropped.',$Ae,'View has been created.',$a,$D);}}if(!$_POST&&$a!=""){$K=adm_view($a);$K["name"]=$a;$K["materialized"]=($uf!="VIEW");if(!$n)$n=error();}page_header(($a!=""?'Alter view':'Create view'),$n,array("table"=>$a),h($a));echo'
<form action="" method="post">
...
';}elseif(isset($_GET["sequence"])){$fa=$_GET["sequence"];$K=$_POST;if($_POST&&!$n){$A=substr(ME,0,-1);$D=trim($K["name"]);if($_POST["drop"])query_adm_redirect("DROP SEQUENCE ".idf_escape($fa),$A,'Sequence has been dropped.');elseif($fa=="")query_adm_redirect("CREATE SEQUENCE ".idf_escape($D),$A,'Sequence has been created.');elseif($fa!=$D)query_adm_redirect("ALTER SEQUENCE ".idf_escape($fa)." RENAME TO ".idf_escape($D),$A,'Sequence has been altered.');else
adm_redirect($A);}page_header($fa!=""?'Alter sequence'.": ".h($fa):'Create sequence',$n);if(!$K)$K["name"]=$fa;echo'
<form action="" method="post">
<p><input name="name" value="',h($K["name"]),'" autocapitalize="off">
<input type="submit" value="Save">
';if($fa!="")echo"<input type='submit' name='drop' value='".'Drop'."'>".confirm(sprintf('Drop %s?',$fa))."\n";echo'<input type="hidden" name="token" value="',$Yh,'">
</form>
';}elseif(isset($_GET["type"])){$ga=$_GET["type"];$K=$_POST;if($_POST&&!$n){$A=substr(ME,0,-1);if($_POST["drop"])query_adm_redirect("DROP TYPE ".idf_escape($ga),$A,'Type has been dropped.');else
query_adm_redirect("CREATE TYPE ".idf_escape(trim($K["name"]))." $K[as]",$A,'Type has been created.');}page_header($ga!=""?'Alter type'.": ".h($ga):'Create type',$n);if(!$K)$K["as"]="AS ";echo'
<form action="" method="post">
<p>
';if($ga!="")echo"<input type='submit' name='drop' value='".'Drop'."'>".confirm(sprintf('Drop %s?',$ga))."\n";else{echo"<input name='name' value='".h($K['name'])."' autocapitalize='off'>\n";textarea("as",$K["as"]);echo"<p><input type='submit' value='".'Save'."'>\n";}echo'<input type="hidden" name="token" value="',$Yh,'">
</form>
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$D=$_GET["name"];$ji=trigger_options();$K=(array)trigger($D)+array("Trigger"=>$a."_bi");if($_POST){if(!$n&&in_array($_POST["Timing"],$ji["Timing"])&&in_array($_POST["Event"],$ji["Event"])&&in_array($_POST["Type"],$ji["Type"])){$cf=" ON ".table($a);$dc="DROP TRIGGER ".idf_escape($D).($y=="pgsql"?$cf:"");$B=ME."table=".urlencode($a);if($_POST["drop"])query_adm_redirect($dc,$B,'Trigger has been dropped.');else{if($D!="")queries($dc);queries_adm_redirect($B,($D!=""?'Trigger has been altered.':'Trigger has been created.'),queries(create_trigger($cf,$_POST)));if($D!="")queries(create_trigger($cf,$K+array("Type"=>reset($ji["Type"]))));}}$K=$_POST;}page_header(($D!=""?'Alter trigger'.": ".h($D):'Create trigger'),$n,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table cellspacing="0">
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$n){$be=0;foreach((array)$_POST["kill"]as$X){if(kill_process($X))$be++;}queries_adm_redirect(ME."processlist=",lang(array('%d process has been killed.','%d processes have been killed.'),$be),$be||!$_POST["kill"]);}page_header('Process list',$n);echo'
<form action="" method="post">
...
as$z=>$X){if(in_array(idf_escape($z),$M))unset($ti[$z]);}if($_POST["export"]){adm_cookie("adminer_import","output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));dump_headers($a);$b->dumpTable($a,"");if(!is_array($_POST["check"])||$ti===array())$H="SELECT $ed$Ri$kd";else{$pi=array();foreach($_POST["check"]as$X)$pi[]="(SELECT".limit($ed,"\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$p).$kd,1).")";$H=implode(" UNION ALL ",$pi);}$b->dumpData($a,"table",$H);exit;}if(!$b->selectEmailProcess($Z,$ad)){if($_POST["save"]||$_POST["delete"]){$I=true;$za=0;$O=array();if(!$_POST["delete"]){foreach($f
as$D=>$X){$X=process_input($p[$D]);if($X!==null&&($_POST["clone"]||$X!==false))$O[idf_escape($D)]=($X!==false?$X:idf_escape($D));}}if($_POST["delete"]||$O){if($_POST["clone"])$H="INTO ".table($a)." (".implode(", ",array_keys($O)).")\nSELECT ".implode(", ",$O)."\nFROM ".table($a);if($_POST["all"]||($ti===array()&&is_array($_POST["check"]))||$Pd){$I=($_POST["delete"]?$bc->delete($a,$Ri):($_POST["clone"]?queries("INSERT $H$Ri"):$bc->update($a,$O,$Ri)));$za=$g->affected_rows;}else{foreach((array)$_POST["check"]as$X){$Ni="\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$p);$I=($_POST["delete"]?$bc->delete($a,$Ni,1):($_POST["clone"]?queries("INSERT".limit1($H,$Ni)):$bc->update($a,$O,$Ni)));if(!$I)break;$za+=$g->affected_rows;}}}$Ae=lang(array('%d item has been affected.','%d items have been affected.'),$za);if($_POST["clone"]&&$I&&$za==1){$ge=last_id();if($ge)$Ae=sprintf('Item%s has been inserted.'," $ge");}queries_adm_redirect(remove_from_uri($_POST["all"]&&$_POST["delete"]?"page":""),$Ae,$I);if(!$_POST["delete"]){edit_form($a,$p,(array)$_POST["fields"],!$_POST["clone"]);page_footer();exit;}}elseif(!$_POST["import"]){if(!$_POST["val"])$n='Ctrl+click on a value to modify it.';else{$I=true;$za=0;foreach($_POST["val"]as$ri=>$K){$O=array();foreach($K
as$z=>$X){$z=bracket_escape($z,1);$O[idf_escape($z)]=(preg_match('~char|text~',$p[$z]["type"])||$X!=""?$b->processInput($p[$z],$X):"NULL");}$I=$bc->update($a,$O," WHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($ri,$p),!($Pd||$ti===array())," ");if(!$I)break;$za+=$g->affected_rows;}queries_adm_redirect(remove_from_uri(),lang(array('%d item has been affected.','%d items have been affected.'),$za),$I);}}elseif(!is_string($Qc=get_file("csv_file",true)))$n=upload_error($Qc);elseif(!preg_match('~~u',$Qc))$n='File must be in UTF-8 encoding.';else{adm_cookie("adminer_import","output=".urlencode($ya["output"])."&format=".urlencode($_POST["separator"]));$I=true;$qb=array_keys($p);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$Qc,$te);$za=count($te[0]);$bc->begin();$Vg=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));$L=array();foreach($te[0]as$z=>$X){preg_match_all("~((?>\"[^\"]*\")+|[^$Vg]*)$Vg~",$X.$Vg,$ue);if(!$z&&!array_diff($ue[1],$qb)){$qb=$ue[1];$za--;}else{$O=array();foreach($ue[1]as$t=>$mb)$O[idf_escape($qb[$t])]=($mb==""&&$p[$qb[$t]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$mb))));$L[]=$O;}}$I=(!$L||$bc->insertUpdate($a,$L,$Wf));if($I)$I=$bc->commit();queries_adm_redirect(remove_from_uri("page"),lang(array('%d row has been imported.','%d rows have been imported.'),$za),$I);$bc->rollback();}}}$_h=$b->tableName($S);if(is_ajax()){page_headers();ob_start();}else
